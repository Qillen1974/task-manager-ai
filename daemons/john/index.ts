...
const log = createLogger("john.main");
...
const llm = new MiniMaxClient(config.MINIMAX_API_KEY);
...
function shutdown(signal: string): void {
  ...
  // Wait for in-progress tasks (with 60s timeout)
...
async function main(): Promise<void> {
  ...
  log.info(`John daemon running. Polling every ${config.POLL_INTERVAL_MS / 1000}s. Press Ctrl+C to stop.`);
}
