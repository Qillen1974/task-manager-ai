...
export default function WorkspacePanel({ teamId }: WorkspacePanelProps) {
  ...
  // Define loadWorkspace with useCallback, only depends on teamId
  // We exclude 'api' from dependencies to prevent infinite loops
  ...
  }, [teamId]); // Only depend on teamId, NOT api

  useEffect(() => {
    loadWorkspace();
  }, [teamId, loadWorkspace]);
